...
import {
  ...
  GET_PLACEMENT_TEST,
  CREATE_RESULT,
  GET_SUBJECT_BY_PLACEMENT,
  GET_PLACEMENTID_BY_STUDENT,
} from "../../../Schema/ApolloSchema";
import { useQuery, useMutation } from "@apollo/client";
import { useRouter } from "next/router";

export default function AddProgrammeBtn({ studentId, setRefetch }) {
  ...
  const [resultCreate, { loading, error }] = useMutation(CREATE_RESULT, {
    onCompleted: ({ resultCreate }) => {
      console.log("resultCreate:->", loading, resultCreate);
    },
  });
    ...
    <>
      ...
      >
        ...
        <DrawerContent>
          ...
          <DrawerBody>
            <Text color="#707070" fontSize="14px" mt="20px" mb="10px">
              Programme
            </Text>
            <Select
            ...
            >
              {levelId &&
                placements &&
                placements?.getPlacementByLevel
                  ?.filter(
                    (grad) => grad._id !== gradeCreated.getPlacementidByStu
                  )
                  .map((plac) => (
                    <option key={plac._id} value={plac._id}>
                      {plac?.name}
                    </option>
                  ))}
            </Select>
          ...
          <DrawerFooter>
            ...
            <Button
              colorScheme="blue"
              type="submit"
              isDisabled={programmeId && levelId && PlacementId ? false : true}
              onClick={() => {
                if (subject) {
